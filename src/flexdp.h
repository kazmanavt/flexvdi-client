...
#define FLEXVDI_RESET 0
#define FLEXVDI_PRINTJOB 1
#define FLEXVDI_PRINTJOBDATA 2
#define FLEXVDI_SHAREPRINTER 3
#define FLEXVDI_UNSHAREPRINTER 4

#define FLEXVDI_CAPABILITIES 0
#define FLEXVDI_CAP_PRINTING 1

#define FLEXVDI_CAP_POWEREVENT 2
#define FLEXVDI_MAX_MESSAGE_LENGTH 65536

#define FLEXVDI_MAX_MESSAGE_TYPE 100


struct FlexVDIMessageHeader_S {
    unsigned size;
    int type;
};
typedef struct FlexVDIMessageHeader_S FlexVDIMessageHeader;

//struct Caps_S {};

struct FlexVDICapabilitiesMsg_S {
    int caps[100];
};
typedef struct FlexVDICapabilitiesMsg_S FlexVDICapabilitiesMsg;

struct FlexVDIResetMsg_S {};
typedef struct FlexVDIResetMsg_S FlexVDIResetMsg;

struct FlexVDIPrintJobMsg_S {
    int id;
    char *options;
    unsigned optionsLength;
};
typedef struct FlexVDIPrintJobMsg_S FlexVDIPrintJobMsg;

struct FlexVDIPrintJobDataMsg_S {
    int id;
    void *data;
    unsigned dataLength;
};
typedef struct FlexVDIPrintJobDataMsg_S FlexVDIPrintJobDataMsg;

struct FlexVDISharePrinterMsg {
    unsigned printerNameLength;
    unsigned ppdLength;
    char *data;
};
typedef struct FlexVDISharePrinterMsg FlexVDISharePrinterMsg;

struct FlexVDIUnsharePrinterMsg {
    unsigned printerNameLength;
    char *printerName;
};
typedef struct FlexVDIUnsharePrinterMsg FlexVDIUnsharePrinterMsg;




void marshallMessage(int type, void* buffer, size_t size);
int unmarshallMessage(int type, void* buffer, unsigned size);
void marshallHeader(FlexVDIMessageHeader *head);
void unmarshallHeader(FlexVDIMessageHeader *head);
void setCapability(FlexVDICapabilitiesMsg*, int);
int supportsCapability(FlexVDICapabilitiesMsg*, int);
